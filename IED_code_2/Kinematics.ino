...
float turnAngleToWheelRev(float turnAngle, float driveBase, float wheelDiameter){
  ...
  float outputRevs = turnAngle * degreeChangePerWheelRev
  return outputRevs;
}

// Converts input travel distance to the required number of wheel rotations to achieve that distance
float distanceToWheelRev(float travelDistance, float wheelDiameter){
  float travelDistancePerWheelRev = wheelDiameter * PI();
  float outputRevs = travelDistance / travelDistancePerWheelRev;
  return outputRevs;
}
